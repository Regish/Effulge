...
from effulge import summarize_variance


def save_variance_report(variance_df, source_df, target_df, super_key, file_name, **kwargs):
    """
    ...
    for column in _list_of_attributes :
        _focused_spark_df = _focused_variance_for_each_attribute(variance_df, source_df, target_df, super_key, column, **kwargs)
...
def _sort_by_most_impacted_attributes(variance_df):
    ...
    and sort them based on thier severity
...
def _focused_variance_for_each_attribute(variance_df, source_df, target_df, 
                                        primary_key_list, attribute_name, 
                                        src_prefix='src', tgt_prefix='tgt'):
    """
    Function to fetch all variances for a a given column.
    ...
    """
    _filtered_records = variance_df.where(f"ARRAY_CONTAINS( EFFULGE_VARIANCE_PROVOKER, '{attribute_name}')")
    focused_df = _filtered_records\
        .join(
            source_df.selectExpr(*primary_key_list, f"{attribute_name} as _{src_prefix}_{attribute_name}")  ,
            [ *primary_key_list ] ,
            "left"
        ).join(
            target_df.selectExpr(*primary_key_list, f"{attribute_name} as _{tgt_prefix}_{attribute_name}")  ,
            [ *primary_key_list ] ,
            "left"
        ).selectExpr(
            *primary_key_list , f"_{src_prefix}_{attribute_name}" , f"_{tgt_prefix}_{attribute_name}"
        )
    #
    return focused_df
